...
```text
{"name": "Mr. Cute", "image": "https://thumbs.gfycat.com/EquatorialIckyCat-max-1mb.gif", "votes":0}
```

The data above is a `String`, containing what looks like
key/value pairs. The keys and values are both wrapped in quotes
...
page, you'll see a **Resources** section that lists one resource: `/characters`.
The server has read the `db.json` file and found our `characters` key, turning it
into a resource. Click `/characters` and you will be navigated to a new page,
`http://localhost:3000/characters`. Instead of a page of info, you'll see the
value associated with `characters` in our data, an array containing two objects:
...
following [RESTful conventions][rest]. By providing `/characters` followed by `/1`
in our URL, JSON Server knows we're asking for a resource called `characters`, and
within that resource, we're asking for whatever data has an ID of `1`. The
`characters` content we store in our JSON file could be in any order. JSON Server
will look through and match the request to an ID and return _that_ content. If
we change to `2`, we'll get the other data we stored in `characters`.
...
```text
GET /characters/1 200 25.666 ms
```

JSON Server sees that this is a GET request. It also notes that the request is
specifically for `/characters/1`. `200` is a [HTTP status code][status] that
...
`/characters` and get everything available from the server. In complex webpages,
...
- Any top-level key/value pair (like `characters` in our example) will be
